...
def gdisconnect():
    ...
    obj = json.loads(credentials, object_hook=lambda d: Namespace(**d))
...
@app.route('/category/<int:category_id>/list/<int:list_list_id>/JSON')
def listItemJSON(category_id, list_list_id):
    List_Item = session.query(ListItem).filter_by(id=list_list_id).one()
...
def categoriesWithItemsJSON():
    ...
    return jsonify(categories=[r.serializeWithItem(session.query(ListItem).filter_by(category_id=r.id).all()) for r in categories])
...
def showList(category_id):
    ...
    if 'user_id' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicList.html', items=items, category=category, creator=creator, user = user)
    else:
        return render_template('list.html', items=items, category=category, creator=creator, user = user)
...
# Edit a list item
@app.route('/category/<int:category_id>/list/<int:list_id>', methods=['GET'])
def showListItem(category_id, list_id):
    item = session.query(ListItem).filter_by(id=list_id).one()
    category = session.query(Category).filter_by(id=category_id).one()
    user_loggedin = False
    if 'user_id' in login_session:
        user_loggedin = True
    return render_template('item.html', category_id=category_id, list_id=list_id, item=item, user_loggedin=user_loggedin)
